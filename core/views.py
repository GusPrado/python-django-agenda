...
def submit_evento(request):
    if request.POST:
        ...
        if id_evento:
            ...
            if evento_editar.usuario == usuario:
                evento_editar.titulo = titulo
                evento_editar.descricao = descricao
                evento_editar.data_evento = data_evento
                evento_editar.save()
...
def json_lista_eventos(request):
    ...
    # eventos = Evento.objects.all()
    dados = {'eventos': eventos}
    return JsonResponse(list(eventos), safe=False) #Safe allows list response
